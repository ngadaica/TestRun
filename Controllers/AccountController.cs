...
namespace TrueTestRun.Controllers
{
    public class AccountController : Controller
    {
        ...
        // Helper ?? danh sách cho dropdown
        private void PopulateRegisterDrops()
        {
            ViewBag.DeptCodes = new SelectList(new[] { "EPE-EE", "EPE-PCB", "EPE-G.M" });
            ViewBag.Titles = new SelectList(new[] { "Qu?n lý trung c?p", "Qu?n lý s? c?p", "G.M", "Director" });
            ViewBag.Factories = new SelectList(new[] { "F1", "F2", "F3", "F4", "F5" });

            // T?o danh sách role v?i text ti?ng Vi?t
            var roleList = new[]
            {
                new { Value = UserRole.Admin.ToString(), Text = "Admin" },
                new { Value = UserRole.DataEntry.ToString(), Text = "Ng??i ghi nh?p" },
                new { Value = UserRole.Approver.ToString(), Text = "Ng??i phê duy?t" }
            };
        ...
        public ActionResult Register(User model)
        {
            ...
            {
                ModelState.AddModelError("ADID", "ADID ?ã t?n t?i");
                PopulateRegisterDrops();
                return View(model);
            }
        ...
        public ActionResult Login(string adid)
        {
            ...
            if (user == null)
            {
                ModelState.AddModelError("", "ADID không t?n t?i");
                return View();
            }
        ...
        // ACTION M?I: T?O ?NH CON D?U
        [AllowAnonymous] // Cho phép truy c?p mà không c?n ??ng nh?p
        public ActionResult UserSeal(string department, string name)
        {
            // L?y user theo tên và phòng ban t? database
