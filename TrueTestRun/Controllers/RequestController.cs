    ...
    public class RequestController : Controller
    {
        ...
        public ActionResult Create()
        {
            ...
            if (currentUser == null || !IsStaff(currentUser))
                return new HttpStatusCodeResult(403, GetResourceString("OnlyStaffCanCreate"));
        ...
        public ActionResult Create(Request model, IEnumerable<HttpPostedFileBase> documentFiles)
        {
            ...
            if (!System.IO.File.Exists(excelPath))
            {
                ...
                {
                    TempData["ErrorMessage"] = GetResourceString("ExcelTemplateNotFound");
                    return View(model);
                }
            }
            ...
            try
            {
                            ...
                            if (file != null && file.ContentLength > 0)
                            {
                                try
                                {
                                    ...
                                    else
                                    {
                                        ...
                                        uploadErrors.Add(GetResourceString("CannotUploadFile").Replace("{0}", file.FileName));
                                    }
                                }
                                catch (Exception fileEx)
                                {
                                    System.Diagnostics.Debug.WriteLine($"[RequestController] Error uploading document {file.FileName}: {fileEx.Message}");
                                    uploadErrors.Add(GetResourceString("ErrorUploadingFile").Replace("{0}", file.FileName).Replace("{1}", fileEx.Message));
                                }
                            }
                        }
                    }
                ...
                string successMessage = GetResourceString("RequestCreatedSuccessfully").Replace("{0}", request.RequestID);
                if (uploadErrors.Any())
                {
                    successMessage += " " + GetResourceString("SomeFilesFailedUpload").Replace("{0}", string.Join(", ", uploadErrors));
                }
            ...
            catch (Exception ex)
            {
                ...
                string errorMessage = GetResourceString("ErrorCreatingRequest");
        ...
        private void ProcessEditStep0(Request request)
        {
            ...
            // SỬA: Logic xử lý cho edit form mà không có prefix Fields[]
            var fieldsList = new List<RequestField>();

            // Lấy tất cả dữ liệu từ Request.Form (không có prefix)
            foreach (string key in Request.Form.AllKeys)
            {
                // Bỏ qua các field hệ thống
                if (!key.StartsWith("__") && key != "RequestID" && key != "EditStepIndex")
                {
                    var value = Request.Form[key];
        ...
        public ActionResult PreviewImage(string id)
        {
            ...
            {
                return HttpNotFound(GetResourceString("InvalidRequestID"));
            }

            var req = fs.LoadRequest(id);
            if (req == null)
            {
                return HttpNotFound(GetResourceString("RequestNotFound"));
            }

            // SỬA: Kiểm tra xem có dữ liệu preview trong session không (cho chế độ edit/live preview)
            var sessionFields = Session["LivePreviewFields"] as Dictionary<string, string>;
            if (sessionFields != null && sessionFields.Any())
            {
                // Sử dụng dữ liệu từ session để tạo preview (cho live editing)
                var tempFieldsList = sessionFields.Select(kvp => new RequestField { Key = kvp.Key, Value = kvp.Value }).ToList();
                var tempRequest = new Request
                {
                    RequestID = req.RequestID,
                    Fields = tempFieldsList,
                    History = req.History ?? new List<WorkflowStep>(),
            ...
            if (!System.IO.File.Exists(excelPath))
            {
                ...
                {
                    return HttpNotFound(GetResourceString("ExcelTemplateNotFound"));
                }
            }
            ...
            try
            {
                ...
                {
                    return HttpNotFound(GetResourceString("CannotCreatePreview"));
                }
            ...
            {
                return HttpNotFound(GetResourceString("PreviewError"));
            }
        }
        ...
        public ActionResult LivePreviewData()
        {
            try
            {
                ...
                // Process all Fields[...] form data (for Create form)
                foreach (string key in Request.Form.AllKeys)
                {
                    if (key.StartsWith("Fields[") && key.EndsWith("]"))
                    {
                ...
                // SỬA: Xử lý form data trực tiếp (for Edit form) - không có prefix Fields[]
                if (!fieldsDict.Any())
                {
                    foreach (string key in Request.Form.AllKeys)
                    {
                        if (!key.StartsWith("__") && key != "RequestID" && key != "EditStepIndex") // Bỏ qua các field hệ thống
                        {
                            var value = Request.Form[key];
                            if (!string.IsNullOrEmpty(value))
                            {
                ...
                // Store in session for PreviewImage/GeneratePreviewImage to use
                Session["LivePreviewFields"] = fieldsDict;

                // Debug log
                System.Diagnostics.Debug.WriteLine($"[LivePreviewData] Updated session with {fieldsDict.Count} fields");
                foreach (var kvp in fieldsDict.Take(5))
                {
                    System.Diagnostics.Debug.WriteLine($"[LivePreviewData] Field: {kvp.Key} = {kvp.Value}");
                }
            ...
            {
                System.Diagnostics.Debug.WriteLine($"[LivePreviewData] Error: {ex.Message}");
                Response.ContentType = "application/json";
